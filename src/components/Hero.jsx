    ...
    return (
        <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden bg-[#030712]">
            ...
            <div className="container mx-auto px-6 relative z-10 text-center">
                ...
                >
                    <div className="relative rounded-2xl bg-[#0F172A]/90 border border-white/10 p-2 backdrop-blur-xl shadow-2xl overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-tr from-[#00C8FF]/5 to-transparent pointer-events-none"></div>

                        {/* Mock UI Header */}
                        <div className="h-12 border-b border-white/10 flex items-center justify-between px-6 bg-[#030712]/50">
                            <div className="flex gap-2">
                                <div className="w-3 h-3 rounded-full bg-red-500/50"></div>
                                <div className="w-3 h-3 rounded-full bg-yellow-500/50"></div>
                                <div className="w-3 h-3 rounded-full bg-green-500/50"></div>
                            </div>
                            <div className="text-xs text-[#64748B] font-mono">CIRVION_DASHBOARD_V2.0</div>
                        </div>

                        {/* Mock UI Content */}
                        <div className="p-8 grid grid-cols-12 gap-6 h-[400px] overflow-hidden text-left">
                            {/* Sidebar */}
                            <div className="col-span-3 border-r border-white/10 pr-6 space-y-4 hidden md:block">
                                {[1, 2, 3, 4, 5].map((i) => (
                                    <div key={i} className="h-10 rounded-lg bg-white/5 w-full animate-pulse" style={{ animationDelay: `${i * 0.1}s` }}></div>
                                ))}
                            </div>
                            ...
                            <div className="col-span-12 md:col-span-9 space-y-6">
                                <div className="flex justify-between items-center mb-8">
                                    ...
                                    <div className="h-10 w-32 bg-[#00C8FF]/20 rounded-lg border border-[#00C8FF]/30"></div>
                                </div>

                                <div className="grid grid-cols-3 gap-6 mb-8">
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className="h-32 rounded-xl bg-[#1E293B]/50 border border-white/5 p-4 relative overflow-hidden group">
                                ...
                                <div className="h-48 rounded-xl bg-[#1E293B]/50 border border-white/5 p-6 relative">
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="text-[#00C8FF] font-mono text-sm animate-pulse">Processing Real-Time Data...</div>
                                    </div>
                                    {/* Animated Graph Line */}
                                    <svg className="absolute bottom-0 left-0 w-full h-24 stroke-[#00C8FF] stroke-2 fill-none opacity-50" viewBox="0 0 100 20" preserveAspectRatio="none">
                    ...
                    {/* Reflection */}
                    <div className="absolute -bottom-20 left-0 right-0 h-20 bg-gradient-to-b from-[#00C8FF]/5 to-transparent blur-xl transform scale-y-[-1] opacity-50"></div>
                </motion.div>
            </div>
        </section>
    );
